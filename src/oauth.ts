...
function loginPage(id:string,clientName:string,redirectUri:string) {
  ...
  return new Response(page,{headers:{
    ...
    // Chromium checks form-action on the 303 callback as well as the initial POST.
    // The callback is already registered/validated; serialize it before use in a header.
    'Content-Security-Policy':`default-src 'none'; style-src 'unsafe-inline'; form-action 'self' ${new URL(redirectUri).href}; frame-ancestors 'none'; base-uri 'none'`,
