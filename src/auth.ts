...
export async function authenticate(request: Request, db: D1Database) {
  const match = /^Bearer ((?:hph|hpo)_[A-Za-z0-9_-]{43})$/i.exec(request.headers.get('Authorization') ?? '');
  if (!match) return null;
  const hash = await sha256(match[1]);
  if (match[1].startsWith('hpo_')) return db.withSession('first-primary').prepare(`
