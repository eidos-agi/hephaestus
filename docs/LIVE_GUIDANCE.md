...
The form posts the personal token only to Hephaestus. Its Content Security Policy also permits the validated, registered callback URL because Chromium can apply `form-action` to the subsequent redirect. The callback uses HTTP 303, so the browser returns with a GET and does not forward the token POST body. A policy restricted to `'self'` could approve a request and clear its cookie while blocking the callback; another click would then report a missing cookie. Open a fresh connection page after updating this policy, since an already loaded page retains its original policy. See [form-action and redirects](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy/form-action).
